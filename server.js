...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const isMatch = await user.comparePassword(password); // Assuming you have a method for this

        if (isMatch) {
            // Successful login, redirect to home.html
            return res.redirect('/home.html'); // This will redirect the user to home.html
        } else {
            return res.status(401).send('Invalid email or password');
        }
    } catch (error) {
